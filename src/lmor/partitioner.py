...
def partition_any_grid(grid, num_intervals=(2, 2), codim=2, dmask=None):
    ...
    def on_x0_boundary(coord):
        ...
        if (np.isclose(np.round(x0_in_increments), x0_in_increments) and x0_in_increments > 0.5
                and x0_in_increments < num_intervals[0] - 0.5):
            return True
        else:
            return False
    ...
    def on_x1_boundary(coord):
        ...
        if (np.isclose(np.round(x1_in_increments), x1_in_increments) and x1_in_increments > 0.5
                and x1_in_increments < num_intervals[1] - 0.5):
            return True
        else:
            return False
    ...
    for dofnr, coord in enumerate(grid.centers(codim)):
        if on_x0_boundary(coord):
            if on_x1_boundary(coord):
                vertices[int(np.round((coord[0] - grid.x0_range[0]) / x0_increment) - 1)
                         ][int(np.round((coord[1] - grid.x1_range[0]) / x1_increment) - 1)].append(dofnr)
            else:
                x1_edges[int(np.round((coord[0] - grid.x0_range[0]) / x0_increment) - 1)
                         ][min(int(np.floor((coord[1] - grid.x1_range[0]) / x1_increment)), num_intervals[1] - 1)
                           ].append(dofnr)
        elif on_x1_boundary(coord):
            x0_edges[min(int(np.floor((coord[0] - grid.x0_range[0]) / x0_increment)), num_intervals[0] - 1)
                     ][int(np.round((coord[1] - grid.x1_range[0]) / x1_increment) - 1)
                       ].append(dofnr)
        else:
            subdomains[min(int(np.floor((coord[0] - grid.x0_range[0]) / x0_increment)), num_intervals[0] - 1)
                       ][min(int(np.floor((coord[1] - grid.x1_range[0]) / x1_increment)), num_intervals[1] - 1)
                         ].append(dofnr)
...
def build_subspaces(subdomains, edges, vertices):
    ...
    dtype = {'names': ['id', 'codim', 'patch', 'cpatch', 'hpatch', 'env', 'cenv', 'henv', 'dofs', 'xenv', 'cxenv'],
             'formats': ['int32', 'int8', 'object', 'object', 'object', 'object', 'object', 'object',
