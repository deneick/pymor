...
class NumpyLocalizer(ImmutableInterface):
    ...
    def __init__(self, space, subspaces):
        ...
        self.subspace_dims = np.array([len(s) for s in subspaces])
    ...
    def globalize_vector_array(self, va, ind, sub_ind=None):
        return self.globalize_vector_array2(va.real, ind, sub_ind) + \
            self.globalize_vector_array2(va.imag, ind, sub_ind) * 1j
